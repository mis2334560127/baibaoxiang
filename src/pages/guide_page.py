...
class GuidePage(QWidget):
    ...
    def _build_install_tab(self) -> QWidget:
        ...
        steps = [
            ...
            ("安装 FFmpeg（录屏功能必需）", "屏幕录制功能依赖 FFmpeg 编码器。\n请从 https://ffmpeg.org/download.html 下载 Windows 版本。\n解压后将 bin 目录路径添加到系统环境变量 PATH 中。\n或者将 ffmpeg.exe 放在软件安装目录下。"),
            ("安装 Tesseract OCR（扫描型PDF转换必需）", "扫描型 PDF（图片型）需要 OCR 识别才能转为可编辑 Word。\n请从 https://github.com/UB-Mannheim/tesseract/wiki 下载 Windows 安装包。\n安装时务必勾选中文简体语言包 (Chinese Simplified)。\n安装后无需额外配置，程序会自动查找。"),
...
· 扫描型 PDF：自动使用 OCR 识别文字（需安装 Tesseract）
· 混合型 PDF：文字页直接转换，图片页 OCR 识别
注意事项：OCR 识别速度较慢，质量取决于原图清晰度"""),

            ("🎬 屏幕录制", """操作步骤：
1. 设置帧率（建议 15-30 FPS，录教程可用 30）
2. 选择编码器（H.264 推荐，兼容性最好）
3. 选择录制模式（全屏 / 区域）
4. 点击「开始录制」开始录屏
5. 完成后点击「停止」
6. 录制文件自动保存到「百宝箱输出」文件夹

注意事项：
· 屏幕录制需要安装 FFmpeg（详见安装指南）
· 录制时会在窗口看到计时器走动
· 暂停功能暂未实现，点击停止即结束录制"""),
        ]
    ...
    def _build_faq_tab(self) -> QWidget:
        ...
        faqs = [
            ("Q: 百宝箱会收集我的数据吗？",
             "A: 不会。本软件所有功能均在本地电脑上运行，不会上传任何文件到网络。图片压缩、PDF 转换、屏幕录制全部在本地完成。唯一的网络请求是预留的广告位（默认关闭），且仅在您主动配置后才生效。"),
            ("Q: 为什么压缩后的图片比目标大小大？",
             "A: 算法使用二分搜索逼近目标大小，通常误差在 ±20KB 以内。如果原始图片非常大（如 10MB+），可能需要多次迭代。您也可以尝试「按质量压缩」模式手动控制。"),
            ("Q: PDF 转 Word 后还是图片、无法编辑怎么办？",
             "A: 这说明您的 PDF 是扫描型（图片型），文字实际是图片的一部分。本软件会自动检测并切换为 OCR 识别模式。使用 OCR 模式需要安装 Tesseract OCR 引擎（详见安装指南第 5 步）。安装后重新转换即可获得可编辑文字。如果 OCR 效果不佳，可以勾选「强制 OCR 识别」重试。"),
            ("Q: 屏幕录制报错「找不到 FFmpeg」怎么解决？",
             "A: 录制功能依赖 FFmpeg。请下载 Windows 版 FFmpeg，将 bin 目录路径添加到系统环境变量 PATH，或直接将 ffmpeg.exe 放在软件目录下，重启软件即可。"),
            ("Q: 可以自定义输出路径吗？",
