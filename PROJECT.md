...
Windows 桌面效率小工具 · 图片批量压缩 / PDF 转 Word / 屏幕录制 · 全部本地运行

## 技术栈

- **语言**: Python 3.13
- **UI框架**: PyQt6
- **核心库**: Pillow (图片处理) / pdf2docx (PDF转换) / pywin32 (屏幕捕获) / FFmpeg (视频编码)
...
│   ├── worker_threads.py    # 后台线程 (压缩/转换/录制/广告)
│   ├── main_window.py       # 主窗口 (侧边栏+页面切换+主题)
│   ├── modules/
│   │   ├── image_compressor.py   # 图片压缩 (二分搜索逼近算法)
│   │   ├── pdf_converter.py      # PDF→Word (pdf2docx)
│   │   ├── screen_recorder.py    # 屏幕录制 (GDI+FFmpeg管道)
...
| PDF 转 Word | pdf2docx 转换器，保留格式和图片 |
| 屏幕录制 | pywin32 GDI 逐帧截图 → FFmpeg 管道实时编码 H.264 MP4 |
